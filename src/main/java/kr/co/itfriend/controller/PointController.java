...
public class PointController {
	...
	@GetMapping("/ChargeSuccess/{amount}/{user_id}")
	public String chargeSuccess(@PathVariable int amount, @PathVariable String user_id) {    
	    // amount 값과 merchant_id 값을 이용해 할 일을 처리하는 코드 작성
	    try {
	        String payment_tool = "card";
	        System.out.println(amount);
	        System.out.println(user_id);
	        point_mapper.insertChargeHistory(user_id, amount, payment_tool); 
	    } catch (Exception e) {
	        // TODO: 예외 처리
	    	e.printStackTrace();
	    }
	...
	public String secondPwdPage_post(@RequestParam String pwd1, @RequestParam String pwd2) throws Exception {
        ...
        if(isValidPwd) {
        	String MB_SecondPwd = PasswordEncryptorService.encrypt(pwd1);
        	
        	try {
				
        		bankMapper.updateSecondPwd(MB_SecondPwd);
			} catch (Exception e) {
