...
public class SpringConfigClass implements WebApplicationInitializer {
   ...
   //Spring Framework 프로젝트 설정을 위한 클래스의 객체 생성
   AnnotationConfigWebApplicationContext servletAppContext =new AnnotationConfigWebApplicationContext();
   servletAppContext.register(ServletAppContext.class);
   
   //요청 발생 시 요청을 처리하는 서블릿을 DispatcherServlet으로 설정 
   DispatcherServlet dispatcherServlet=new DispatcherServlet(servletAppContext); //매개변수로 선언된
  //servletContext객체를 이용하여 servlet추가 ServletRegistration.Dynamic
  ...
  AnnotationConfigWebApplicationContext rooAppContext=new AnnotationConfigWebApplicationContext();
   ...
   filter.setInitParameter("encoding", "UTF-8"); //dispatcher에 의해서 편집한 경로 UTF-8로
...
public class SpringConfigClass extends AbstractAnnotationConfigDispatcherServletInitializer {
    ...
    //52428800 : 파일데이터를 포함한 전체용량(1024*500) 500M
    //0 : 데이터를 받아서 자동 저장
	@Override
